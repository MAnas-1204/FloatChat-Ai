...
export default function DemoPage() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    {
      ...
      role: "ai",
      content:
        "Hi! Ask me anything about ARGO floats, temperature or salinity trends.",
      kind: "text",
    },
  ]);
  ...
  const send = () => {
    ...
    setTimeout(() => {
      const variants: Msg[] = [
        {
          id: crypto.randomUUID(),
          role: "ai",
          content: "Here is a quick temperature trend preview.",
          kind: "chart",
        },
        {
          id: crypto.randomUUID(),
          role: "ai",
          content: "Plotting float density map in the North Atlantic.",
          kind: "map",
        },
        {
          id: crypto.randomUUID(),
          role: "ai",
          content:
            "Profiles from ARGO float 6903017 show stable salinity at 1000m.",
          kind: "profiles",
        },
        {
          id: crypto.randomUUID(),
          role: "ai",
          content:
            "Median trend since 2010 is +0.08°C/decade around 45°N, 30°W.",
          kind: "text",
        },
      ];
      ...
      setIsTyping(false);
      if (pick.kind && pick.kind !== "text")
        setTab(pick.kind === "chart" ? "chart" : pick.kind);
    }, 1100);
  };
    ...
    () => (
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-foreground/70 max-w-2xl mx-auto">
          Ask about ocean data and instantly visualize maps, charts, and
          profiles.
        </p>
      </div>
    ),
        ...
        <div className="container py-8">
              ...
              <Tabs value={tab} onValueChange={setTab}>
                ...
                >
                  <div className="relative grid gap-4 md:grid-cols-[1fr]">
                    <div
                      ref={listRef}
                      className="h-[420px] overflow-y-auto rounded-xl border border-brand-aqua-start/30 dark:border-white/10 bg-[#f8f9fb] dark:bg-white/5 p-4"
                    >
                      {messages.map((m) => (
                        <div
                          key={m.id}
                ...
                <TabsContent
                  value="map"
                  className="mt-4 transition-all duration-300 data-[state=inactive]:opacity-0 data-[state=active]:opacity-100"
                >
                      ...
                      >
                        <defs>
                          ...
                          >
                            <stop
                              offset="5%"
                              stopColor="hsl(var(--brand-aqua-start))"
                              stopOpacity={0.5}
                            />
                            <stop
                              offset="95%"
                              stopColor="hsl(var(--brand-aqua-end))"
                              stopOpacity={0.05}
                            />
                          </linearGradient>
                        </defs>
                        ...
                        <ReTooltip
                          cursor={{
                            stroke: "hsl(var(--brand-aqua-start))",
                            strokeWidth: 1,
                          }}
                        />
                        <Area
                ...
                >
                  <div className="grid gap-4 sm:grid-cols-2">
                    {["6903017", "7900690", "4903593", "3901855"].map(
                      (id, i) => (
                        <div
                          key={id}
                          className="rounded-xl border border-brand-aqua-start/30 dark:border-white/10 bg-white/80 dark:bg-white/5 p-4 animate-fade-up"
                        ...
                        >
                          ...
                          <div className="mt-1 text-xs text-foreground/70">
                            Last profile: 2025-02-01 • Region: North Atlantic
                          </div>
          ...
          <section className="mt-10">
            <div className="rounded-3xl border border-brand-aqua-start/30 dark:border-white/10 ring-1 ring-brand-aqua-start/20 dark:ring-white/10 bg-white/50 dark:bg-white/5 p-6 backdrop-blur-sm animate-fade-up">
              <h2 className="text-center text-2xl font-bold tracking-tight inline-flex items-center gap-2 justify-center">
                <Waves className="h-5 w-5 text-brand-cyan" /> Why FloatChat
                Matters
              </h2>
              <div className="mt-6 grid gap-4 sm:grid-cols-3">
                <div className="rounded-2xl border border-brand-aqua-start/20 dark:border-white/10 bg-white/60 dark:bg-white/10 p-5 text-center">
                  4,000+ ARGO floats worldwide
                </div>
                <div className="rounded-2xl border border-brand-aqua-start/20 dark:border-white/10 bg-white/60 dark:bg-white/10 p-5 text-center">
                  Real-time ocean data access
                </div>
                <div className="rounded-2xl border border-brand-aqua-start/20 dark:border-white/10 bg-white/60 dark:bg-white/10 p-5 text-center">
                  AI-powered insights for researchers & students
                </div>
              </div>
            </div>
          </section>
            ...
            <div className="relative overflow-hidden rounded-3xl border border-brand-aqua-start/30 dark:border-white/10 ring-1 ring-brand-aqua-start/20 dark:ring-white/10 bg-white/50 dark:bg-white/5 px-8 py-10 backdrop-blur-sm animate-fade-up">
              ...
              <div className="text-center">
                <div className="inline-flex items-center gap-2 rounded-full border border-brand-aqua-start/30 dark:border-white/10 bg-white/60 dark:bg-white/10 px-3 py-1 text-xs text-foreground/70">
                  <Waves className="h-3.5 w-3.5 text-brand-cyan" /> Oceanauts
                </div>
                <h3 className="mt-3 text-3xl md:text-4xl font-extrabold tracking-tight bg-clip-text text-transparent bg-[linear-gradient(90deg,hsl(var(--brand-aqua-start)),hsl(var(--brand-cyan)),hsl(var(--brand-aqua-end)))] bg-[length:200%_100%] animate-shimmer">
                  Meet the Team
                </h3>
                ...
                {[
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <img
                        src={`https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(name as string)}&backgroundType=gradientLinear&radius=16`}
                        alt={name as string}
